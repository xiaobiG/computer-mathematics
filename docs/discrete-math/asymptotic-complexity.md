...
时间复杂度必须说明操作模型：数组随机访问、哈希查询、整数位宽、缓存和 I/O 都会改变成本。空间也独立计量；归并排序常为 $O(n)$ 辅助空间，递归深度为 $O(\log n)$。复杂度不衡量网络、并行或运行时延迟。

主定理只适用于特定等分递推；不规则递归应使用[递推关系](/discrete-math/recurrences)、递归树或摊还分析。`NaN` 也不属于二指针证明所需全序，实验必须拒绝它而不能只检查相邻 `>`；增长率不能替代基准测试。
