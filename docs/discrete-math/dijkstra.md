...
运行 `python -m unittest projects.algorithm_lab.test_dijkstra_trace`。实现会在开始前验证整张图的节点和权重，而非只在可达分支中偶然发现负边；它使用序号避免同距离、不可比较节点导致堆比较失败。事件按最终确定顺序记录节点、距离和成功松弛。`shortest_path_certificate` 再独立核对：每个有限距离都有同权重的父指针路径，所有边满足松弛不等式，轨迹恰好覆盖可达点且确定距离单调；它还按确定的堆/序号规则重放每一项事件和成功松弛，防止正确终值旁边混入伪造过程。前者给出“能达到这个距离”，后者沿任意路径逐边推出“不能更短”，而事件重放证明展示的过程确实来自该算法，三者合在一起才是最短路证书。
...
1. 每次只记录最小堆弹出的未确定节点；松弛后比较候选距离与旧 `dist`。
2. 从最短路径上第一个不在 $S$ 的顶点倒推其前驱；非负边保证候选长度不会低于已确定距离。
3. 从目标沿 `parent` 回溯并最后反转；不可达节点应保留无父节点/无穷距离，而不是伪造路径。
4. 分别准备一条负边、一个负环和一张全非负图；报告可达性、距离语义和算法是否应拒绝输入。
