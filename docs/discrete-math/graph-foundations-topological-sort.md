...
1. 反设有环，删去环上一边后其余环边仍连接两个端点，因此图仍连通而边数变为少于 $n-1$；再结合连通图至少有 $n-1$ 条边。
2. 奇环沿边交替染色会回到起点并要求同一顶点两色；偶环则可按交替两色闭合。
3. 分量内部的边数应只计两端都在该分量的边；无向邻接表会重复出现边，需规定只在一个端点计数。
4. Kahn 队列为空表示剩余点都没有入度 0；从任一点反复沿入边走，有限顶点中必重复，从重复段读出环。
5. 先约定边是“依赖者指向依赖”还是反向；测试至少固定无环序满足所有依赖、环返回明确失败、孤立包也出现在结果中。
