...
密集候选检查最坏为 $O(n^3)$，矩阵存储为 $O(n^2)$。稀疏两跳扫描为 $\sum_{(x,y)\in R}\operatorname{outdeg}_S(y)$；它常在稀疏图上更小，却可能在高度节点或稠密图上接近立方工作量。若要大量任意对查询，预计算矩阵或闭包可能值得；若仅从少量源点查询，BFS/DFS 往往更合适。
