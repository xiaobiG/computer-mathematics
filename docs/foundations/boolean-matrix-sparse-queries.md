...
你将推导布尔乘积，比较密集、稀疏和位集合批次查询，并按密度、重复源和更新模式选择表示。

## 从两跳查询开始

若有 $(a,b),(b,c)$，两跳查询问是否存在中间点 $b$，不是边权相乘。矩阵直接查格，邻接表沿实际边扩展，语义相同而工作不同。
...
值 1 表示中间点存在，不表示路径数或距离；普通和得到路径数，min-plus 才对应带权最短路。
...
把右关系的每一行打包为位集合后，一个源的结果是其左邻居各行的按位 OR。对 `a,a,b` 批次、4 个点和 2 位模型，`a` 的第二次查询命中缓存；结果仍与稀疏扫描相同，但打包 4 个边并做 6 次模型化字 OR，总成本 10，反而高于本批的 7 次稀疏扫描：
...
这是固定字宽操作模型：真实速度还取决于 CPU、语言、布局和更新失效；缓存只有在足够重复且快照未变时才可能摊回构造成本。

## 失败案例与工程边界

- **普通乘法当可达性。** 它计路径数，不是存在性。
- **稀疏或位集合永远更快。** 热点、构造、缓存和更新都会改变计数。
...
4. **开放**：为共同关注查询比较三种表示，写明密度、批次和更新率。
