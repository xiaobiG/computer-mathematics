...
这里必须区分两个相近但不相同的对象。**传递闭包** $R^+$ 只允许长度至少为 1 的路径：孤立点 $(v,v)$ 不会凭空出现。若算法语义允许“什么也不走也算到达自己”，则需要**自反传递闭包**

$$R^*=I\cup R^+,$$

其中 $I=\{(v,v):v\in D\}$ 是域上的恒等关系。图搜索中的“从 $s$ 出发，$s$ 是否可达”通常采用 $R^*$；本实验的 `closure_pairs` 则刻意报告 $R^+$，所以只有实际回路才会产生自环。必须先声明路径能否为零步。
...
- 将 $R^+$ 与 $R^*$ 混用：前者要求至少一条边，后者额外含全部 $(v,v)$；初始化语义不同。
...
5. 对没有边的域 $D=\{a,b\}$，分别写出 $R^+$ 与 $R^*$；说明哪一个允许零步到达。
...
5. $R^+=\varnothing$，而 $R^*=\{(a,a),(b,b)\}$；后者允许零长度路径。
