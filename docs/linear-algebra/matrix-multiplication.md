...
这里的矩阵合同还要求每个条目为有限、非布尔的实数，并在某次行—列累加上溢为 `inf` 时拒绝。形状正确并不让 $10^{308}\cdot10^{308}$ 成为可解释的实数矩阵条目；把 `inf` 或 `nan` 继续交给后续消元、投影或证书，只会把“公式的有限实数前提”变成无意义的比较。实际大尺度计算可能需要缩放、更高精度或成熟库的溢出策略，而不是把非有限结果伪装为一个矩阵乘积。
