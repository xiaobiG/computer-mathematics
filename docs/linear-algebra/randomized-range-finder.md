...
因此 $P$ 是到 $\mathrm{col}(Q)$ 的正交投影。对每一列 $a_j$，$QQ^Ta_j$ 都是该子空间中距 $a_j$ 最近的向量；把所有列拼起来就是 $QQ^TA$。随机性的职责不是改变投影定理，而是希望草图 $Y=A\Omega$ 的列空间足够接近 $A$ 的主方向。
...
上述投影恒等式证明输出位于 $\mathrm{col}(Q)$，并且对**这个已采样的子空间**是最小二乘最优。秩上界来自 $QQ^TA$ 经过至多 $\ell$ 个正交方向。测试用秩一矩阵验证其随机草图仍落在同一列空间，重构误差接近零；同时用固定种子比较幂迭代前后实测残差，检查并非只宣称“随机通常有效”。
