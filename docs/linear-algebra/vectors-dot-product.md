...
- 用点积计算一个向量在另一个方向上的分量；
- 从投影推导余弦相似度，而不只记住公式；
- 在代码中处理维度不一致和零向量，并根据任务语义选择余弦相似度或欧氏距离。
...
3. **迁移**：运行 `compare_similarity_metrics`，把 `near` 和 `scaled` 改成你自己的二维“用户行为”候选。先预测两种第一名，再解释任务究竟是否应忽略行为总量。
4. **开放**：构造两个方向相同但长度相差一千倍的向量，并增加一个方向略偏、长度接近的候选；分别比较余弦相似度与欧氏距离。说明检索、异常检测和预算监控中应如何根据业务语义选择度量。

## 练习答案提示

1. 先逐分量相乘再求和；点积为零时无需近似比较即可判断正交。
2. 将“抛出 `ValueError`”也当作可验收结果；反向单位向量的分子为 $-1$。
3. 观察 `cosine_ranking` 和 `euclidean_ranking` 是否分歧；预归一化只在向量可复用且不为零时节省重复范数计算。
4. 令 $v=1000u$，再增加一个长度接近 $u$、方向稍偏的候选；结论必须绑定“方向”还是“绝对幅度”这一业务语义。
