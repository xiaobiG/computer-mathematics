...
给定候选密码与记录，按以下顺序执行：先验证记录的算法、轮数、盐和派生值形状；再用记录内的参数重新派生；最后做安全比较。只有比较成功且记录成本低于目标值，才生成新的随机盐和新记录。记录无效要作为系统错误拒绝，候选密码不匹配才返回 `False`；两条路径不能混写。
...
运行 `python -m unittest projects.crypto_toybox.test_password_storage`。模块用不可变记录保存算法、轮数、盐和派生值；验证前会拒绝未知算法、过短盐、非正轮数，以及不是 32 字节的派生值。这个 **fail-closed** 合同把损坏的存储记录与“密码不匹配”区分开：前者是系统/数据完整性错误，应记录并走恢复流程，不能悄悄当作一次普通登录失败。测试固定不同盐，以验证相同密码不产生同一记录，并验证错误密码绝不会触发成本迁移。`salt` 参数仅为确定性课堂测试而暴露，实际调用必须省略它以生成随机盐。这段代码展示记录形状与验证流，不是完整认证系统：示例成本参数不是普适安全建议；生产环境应通过所选库和部署硬件的基准测试确定参数，并交给受审计的身份平台处理会话、恢复、MFA 和监控。
...
- **把损坏记录当成密码错误**：错误的算法标识、盐或派生值长度应 fail closed；把它们吞成 `False` 会掩盖迁移、序列化或数据库损坏问题。
...
3. **编码题**：为示例添加“成功登录后若 rounds 过旧则重新派生”的迁移函数，并测试错误密码不会触发迁移；再为派生值长度错误的记录写一个拒绝测试。
4. **开放题**：为一个高价值账户系统画出在线与离线攻击面，分别分配 KDF、限速、MFA、会话失效和告警措施。

## 练习答案提示

1. 至少有算法/版本、KDF 参数、随机盐、派生摘要和用户/记录标识；盐与参数可公开，摘要不是可恢复密码，pepper 若使用应在独立服务器秘密中管理。
2. 唯一盐使每位用户的同一口令进入不同派生输入，预计算表不能直接跨记录复用；口令本身仍是低熵猜测目标。
3. 仅在验证旧摘要成功后检查参数并重新派生；错误密码绝不能触发迁移或泄漏其是否接近正确；记录字段不符合合同则作为损坏数据拒绝，而不是伪装成密码错误。
4. KDF 对抗离线猜测，限速/MFA 对抗在线攻击，会话失效限制令牌风险，告警支持发现异常；分别写明攻击者能力和失败后的响应。
