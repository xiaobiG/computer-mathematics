...
特别地，把同一个底数重复写成 $[2,2,2]$ 不会把一份证据变成三份：三条平方链完全相同，失败事件也完全相关。本教学 API 因此拒绝重复底数，只让每一轮对应不同的显式底数；但“不同”仍不等于密码学意义的独立、均匀安全随机抽样，故报告不会自行声称获得了 $4^{-r}$ 的生产级概率保证。
...
- **重复同一底数**：相同输入会重放相同平方链，不能把轮数或独立误差界按重复次数放大。
