...
此教学实现只处理有限系数、实根和二次式。它还要求 **binary64 中已经算出的判别式有限**：例如 $b=10^{308}$ 会令 $b^2$ 上溢，合同会在开方和根计算前拒绝，而不是把 `inf` 当作大根或“稳定公式”的证据。缩放系数可保留相同根：对任意非零 $s$，
...
- **把 `inf` 当作根。** 系数有限不保证中间的 $b^2-4ac$ 有限；应缩放或交给具备溢出控制的求根库，本实验直接拒绝。
...
4. **边界题**：解释为何 $(a,b,c)=(1,10^{308},1)$ 的系数有限却仍须被本实验拒绝；写出一个不改变根的缩放变换。
...
4. $b^2$ 会在 binary64 上溢为无穷；任取非零 $s$ 同乘三项不改变方程的根，但缩放策略本身也需要单独审计。
