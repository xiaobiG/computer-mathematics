...
binary64 只能保留有限位尾数，因此转换产生某个相邻格点 $x_1=\operatorname{fl}(0.1)$。把相对这一轮舍入的误差写成 $\delta_1$，就有

$$
x_1=0.1(1+\delta_1),
\qquad |0.1\delta_1|\leq \tfrac12\operatorname{ulp}(x_1).
$$

同理，源代码中的 $0.2$ 与 $0.3$ 分别先成为 $x_2=\operatorname{fl}(0.2)$、$x_3=\operatorname{fl}(0.3)$。表达式 `0.1 + 0.2` 不是把精确分数相加后与精确 $3/10$ 比较，而是再做一次运算舍入：

$$
s=\operatorname{fl}(x_1+x_2).
$$
