...
对一个有限十进制字面量 $x$，设 $\mathrm{fl}(x)$ 是转换后的 binary64 值，局部格距为 $\mathrm{ulp}(\mathrm{fl}(x))$。在默认的“最近值、偶数舍入”模式下（非恰好平分的情形），转换误差满足

$$
|\mathrm{fl}(x)-x|\leq \frac12\mathrm{ulp}(\mathrm{fl}(x)).
$$
...
以 $0.1$ 为例，先将它当作精确有理数 $1/10$，而不是已经存进变量的浮点数。它的二进制小数部分循环：

$$
\frac1{10}=0.0001100110011\ldots{}_2.
$$

binary64 只能保留有限位尾数，因此转换产生某个相邻格点 $x_1=\mathrm{fl}(0.1)$。把相对这一轮舍入的误差写成 $\delta_1$，就有

$$
x_1=0.1(1+\delta_1),
\qquad |0.1\delta_1|\leq \tfrac12\mathrm{ulp}(x_1).
$$

同理，源代码中的 $0.2$ 与 $0.3$ 分别先成为 $x_2=\mathrm{fl}(0.2)$、$x_3=\mathrm{fl}(0.3)$。表达式 `0.1 + 0.2` 不是把精确分数相加后与精确 $3/10$ 比较，而是再做一次运算舍入：

$$
s=\mathrm{fl}(x_1+x_2).
$$

即使 $x_1,x_2$ 各自都是最接近的格点，$x_1+x_2$ 也未必落在 $x_3$ 这个格点上；因此 $s$ 可能与 `0.3` 的存储值不同。这里有三次独立的“贴近格点”步骤，不能把第一次转换的半 ULP 界误当成整个表达式的误差界。实际工程中，应该用误差模型选择比较契约，或对金额等需要精确十进制语义的数据改用整数最小单位/十进制定点。
