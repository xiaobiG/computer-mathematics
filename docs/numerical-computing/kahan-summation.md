...
1. 逐项写出普通累加何时吞掉小数；Kahan 用补偿在下一轮尝试收回，pairwise 的结果取决于树形分组，三者都应用同一精确参考比较。
2. 每层把问题数约减半，直到一个叶子；树高为 $\lceil\log_2 n\rceil$，但总加法次数仍为 $n-1$。
3. 传递索引区间而非切片；空区间返回加法单位元 0，单元素直接返回，奇数长度需明确哪一半多一个元素。
4. 固定分块规则、块内顺序和二叉归约树，并记录数据版本与浮点格式；否则调度变化会改变舍入路径，难以复现差异。
