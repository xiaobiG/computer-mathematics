...
正对角元只是 Jacobi 除法有定义的必要条件，不是正定性的判据。反例

$$
A=\begin{bmatrix}1&2\\2&1\end{bmatrix}
$$

是对称的，两个对角元也都是正数，但它并非正定：对 $v=(1,-1)^T$，有 $v^TAv=-2<0$。教学实现用实 Cholesky 分解将这个前提变成可定位的主元检查。若 $A=LL^T$ 且 $L$ 的对角元为正，则 $A$ 是 SPD；反之，SPD 矩阵存在这样的分解。

对上面的反例，第一步给出 $L_{00}=1$、$L_{10}=2$，第二个待开方主元为

$$
A_{11}-L_{10}^2=1-2^2=-3.
$$

它非正，故不存在实的正对角 Cholesky 因子。这里的 `-3` 比运行到某一轮才报“曲率非正”更直接地说明：问题出在输入并不满足 CG 的定理前提，而不是某个迭代参数。
...
from projects.floating_point_museum.preconditioned_cg import (
    ...
    spd_cholesky_certificate, spd_cholesky_report,
)
...
spd = spd_cholesky_report(indefinite)
assert spd["cholesky_pivots"] == [1.0, -3.0]
assert spd["failing_pivot_index"] == 1
assert not spd["positive_definite"]
assert spd_cholesky_certificate(indefinite, spd)
```

运行 `python -m unittest projects.floating_point_museum.test_preconditioned_cg`。`preconditioner="identity"` 公开了普通 CG 基线，`"jacobi"` 只做对角缩放；两条轨迹均记录 $\alpha$、$\beta$、近似解、残差范数和 $r^Tz$，证书必须以相同预条件选择重放，否则比较没有意义。CG 启动前，`spd_cholesky_report` 还会逐个保存 Cholesky 主元；证书重算该诊断，不能把失败主元改成“正定”。它能拒绝伪造的步长或“看似很小”的残差。稠密 Cholesky 检查为 $O(n^3)$，此教学合同用它在小系统上审计前提；大规模稀疏求解通常不会为每次运行密集分解。CG 本身的稠密每轮为 $O(n^2)$，稀疏实现则主要为一次矩阵—向量乘，接近 $O(\mathrm{nnz}(A))$；Jacobi 还需 $O(n)$ 次逐对角元除法，真实选择应比较总耗时而不是只看迭代数。
...
Jacobi 预条件器最便宜，却无法处理零/负对角，也未必显著聚集特征值。教学实现的 Cholesky 检查只为小型稠密输入 fail-closed；它不应被误作大规模稀疏系统的实际预条件器。只有当较少迭代节省的矩阵—向量乘成本超过预条件应用和构造成本时，“更少步”才可能成为“更快”。真实大规模求解应评估不完全 Cholesky、多重网格或领域专用预条件器，并采用成熟稀疏库。

## 失败案例与常见误区

- **非对称矩阵**：CG 的共轭推导失效，教学实现会拒绝它；改用 GMRES、BiCGSTAB 等合适方法。
- **非正定曲率**：$p^TAp\le0$ 表示 SPD 前提不能在当前方向成立，不能继续除法。
- **正对角的错觉**：$mathrm{diag}(A)>0$ 只保证 Jacobi 的分量除法不除以零；像上述反例仍可有负 Cholesky 主元，故实现会在迭代前拒绝。
...
4. **编码题**：篡改一条 `CgEvent.alpha` 或 Cholesky 报告的失败主元，确认相应证书拒绝；再测试正对角但不正定、非对称输入和未知预条件名称。
...
4. 轨迹证书会以指定的预条件选择完整重算事件；Cholesky 证书会重算每个主元。正对角反例的第二主元是 $-3$，因此拒绝来自 SPD 前提，而非性能优化。
