...
## 失败轨迹：不能因为“这次已经接近真值”就停止

若真实值在实验时已知，最诱人的错误规则是每增加一批样本就看一次，并在首次足够接近时停下：

$$
T=\min\left\{r:\left|\hat\pi_r-\pi\right|\le\varepsilon\right\}.
$$

这个 $T$ 是由结果本身选出来的，故“第 $T$ 次很接近”不是固定样本量估计器的误差保证。真实未知的积分、风险或模型参数更不能用未知真值作停止条件；即使在 $π$ 教学例子中可以这样算，也只能把它当作选择偏差的反例。

`truth_target_stopping_report` 用同一个局部随机流连续延长前缀，不把它伪装成五次独立重试。它完整保存每一阶段的样本数、估计、绝对误差和首次命中位置：

```python
from projects.floating_point_museum.simulation import (
    truth_target_stopping_report,
    truth_target_stopping_report_certificate,
)

report = truth_target_stopping_report(100, 5, seed=2026, tolerance=4.0)
assert report["stage_samples"] == (100, 200, 300, 400, 500)
assert report["first_within_tolerance_stage"] == 1
assert report["stopping_status"] == "stopped_by_truth_target"
assert report["automatic_action"] == "none"
assert truth_target_stopping_report_certificate(
    100, 5, seed=2026, tolerance=4.0, report=report,
)
```

把容差设为零时，有限样本得到的有理数不可能恰等于无理数 $\pi$，报告会诚实返回 `truth_target_not_met`。证书会重放所有前缀，不能把“第一阶段命中”篡改成较晚的幸运时刻。这仍**不是**序贯置信区间、alpha spending、有效样本量诊断或自动停止器；要用基于不确定性的停止规则，必须预先声明误差界、独立性/方差条件与最大预算。
...
- **看真值提前停**：每一批都检查 $|\hat\theta-\theta|$，只展示首次命中，会让被选中的轨迹看起来比预先固定预算时更好。
...
5. “可以持续看结果，命中精度后仍沿用固定样本量区间。”错误：停止时刻依赖结果，须另行使用已声明的序贯推断设计。
...
5. **审计题**：解释为何 $T=\min\{r:|\hat\pi_r-\pi|\le\varepsilon\}$ 不是预先固定样本量。把其改写为一个诚实但仍受限的停止设计时，至少还须声明哪三项信息？
...
5. $T$ 由已观测误差选择，故有选择偏差；至少声明误差界/目标、独立或方差条件、最大样本预算。若要保证覆盖率，还需要相应的序贯方法，而不是沿用普通固定样本公式。
