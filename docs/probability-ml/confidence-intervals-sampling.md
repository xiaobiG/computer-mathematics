...
estimatedMinutes: 75
experiment: "用可复现模拟检验 95% 置信区间的长期覆盖率，并将均值差区间传入最小效应人工复核"
title: 抽样误差、置信区间与覆盖率
...
上游课给出的区间不应在页面末尾消失。下面先把两组观测生成“均值差区间”产物，再把**同一份产物**交给下游的最小效应复核。后者不会重新挑一个点估计，也不会以 p 值替代业务阈值；它只比较完整区间与由责任方声明的最小可接受提升。
...
interval = two_sample_mean_interval(
    [11, 12, 13, 12, 11],  # treatment
    [8, 9, 8, 9, 8],       # control
)
review = practical_effect_review(interval, minimum_effect=2.0)

assert review["interpretation"] == "interval_entirely_at_or_above_declared_minimum_effect"
assert review["automatic_action"] == "none"
```

三种输出必须分开解释：区间整体不低于最小效应，是“现有区间与该门槛一致”；区间整体低于门槛，是“现有区间不支持达到该门槛”；区间穿过门槛，则是“现有信息无法分开两种可能”。它们都不是上线、下线、重训或因果结论。下游会从报告保存的原始有限样本重新计算区间，拒绝被改写的差值、标准误或端点；但它不能验证随机分配、独立分析单位、正态近似或阈值本身是否合理。

这就是正确的数据流：抽样单位和区间来自本课，实际重要门槛来自决策上下文，结果只进入人工复核问题。若随机化单位是城市、用户簇或时间块，不能将逐记录区间直接传过去；应先回到合适的簇级/时间分层重采样设计。
...
5. 区间只量化既定抽样设计下的随机误差；目标总体、抽样框缺失、拒答和测量偏差是不同来源，需单独说明。
6. 比较最小效应与区间两端，而不是只比较点估计：上端低于门槛、下端不低于门槛和两端夹住门槛对应三种不同证据状态；每种状态仍要人工检查阈值、抽样单位和潜在伤害。
