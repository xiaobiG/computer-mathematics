...
读完后，你能从伯努利似然推导二元交叉熵和批量梯度，解释线性分数为何作用在对数赔率上，实现 L2 正则化，用 Brier 分数区分“分类正确”和“概率可信”，并将已校准概率与明确的误报/漏报成本连接为决策阈值。
...
阈值 $0.5$ 隐含了一条政策：误报和漏报成本相同。若模型概率 $p$ 已在相关人群上校准，且误报正类的成本为 $c_{FP}>0$、漏报正类的成本为 $c_{FN}>0$，两种动作的条件期望成本分别是

$$
C(\text{判正}\mid p)=c_{FP}(1-p),
\qquad
C(\text{判负}\mid p)=c_{FN}p.
$$
...
时才严格偏好判正；等号表示两种动作成本相同，必须由额外、预先声明的流程决定，而不是把它伪装成模型结论。

```python
from projects.naive_bayes_spam.cost_sensitive_decisions import cost_sensitive_decision

probability = 0.2
balanced = cost_sensitive_decision(probability, false_positive_cost=1.0, false_negative_cost=1.0)
missed_spam_is_costly = cost_sensitive_decision(
    probability, false_positive_cost=1.0, false_negative_cost=9.0,
)

assert balanced.positive_threshold == 0.5
assert balanced.recommended_label is False
assert missed_spam_is_costly.positive_threshold == 0.1
assert missed_spam_is_costly.recommended_label is True
```
...
另一个常见错误是先把概率阈值化为 $0/1$ 再计算交叉熵：$\log 0$ 无定义，也丢失了梯度。损失函数应使用连续概率，分类阈值只用于最终决策。成本敏感阈值还额外依赖校准、成本口径和适用人群；把训练集分数或任意风险标签直接送入公式，并不能产生正当行动。
...
4. **决策**：对 $p=0.2,0.5,0.8$ 分别比较 $c_{FP}=c_{FN}=1$ 与 $c_{FP}=1,c_{FN}=9$；预测哪些格的建议会翻转，并解释平局为什么不能自动打破。
5. **开放**：把垃圾邮件项目拆为训练、验证、测试集，比较朴素贝叶斯和逻辑回归的 Brier 分数；声明一个与场景匹配的成本口径，并说明比较可能受哪些数据漂移影响。
...
4. 阈值为 $c_{FP}/(c_{FP}+c_{FN})$；相同成本给 $0.5$，漏判成本增大则阈值下降。成本相等时的精确阈值点是并列，不是从数学上推出的默认标签。
5. 按时间或来源切分以避免泄漏，在同一保留测试集计算 Brier 分数和校准曲线；词汇、攻击策略、标签政策和类别比例变化都会导致分布漂移，故需记录切分与再训练策略。
