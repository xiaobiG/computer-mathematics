...
| 3 · 工程与前沿 | 校准、再校准、层级部分汇聚、时间相关方差与区间、漂移监控、带标签性能审计、联合证据、子群体不确定性、分组校准、冻结窗口比较与 bootstrap、失访观测机制、重要性采样、MCMC | 审计不确定性、相关样本、选择偏差与模型分数 | [多组概率的部分汇聚](/probability-ml/hierarchical-beta-binomial) → [两状态 Markov 链方差](/probability-ml/markov-dependent-sample-mean) → [相关样本的置信区间](/probability-ml/dependent-interval-coverage) → [概率校准与可靠性曲线](/probability-ml/calibration-reliability) → [概率再校准](/probability-ml/recalibration) → [数据漂移监控](/probability-ml/data-drift-monitoring) → [带标签窗口性能审计](/probability-ml/labeled-window-performance-degradation) → [联合证据](/probability-ml/joint-input-label-evidence) → [子群体性能](/probability-ml/subgroup-performance-uncertainty) → [分组校准](/probability-ml/subgroup-calibration-uncertainty) → [冻结窗口比较](/probability-ml/frozen-window-calibration-comparison) → [时间块 Bootstrap](/probability-ml/block-bootstrap-calibration) → [簇级 Bootstrap](/probability-ml/cluster-bootstrap-calibration) → [时间分层簇级 Bootstrap](/probability-ml/time-stratified-cluster-bootstrap) → [失访与逆概率加权](/probability-ml/attrition-observation-ipw) |
...
- [时间相关观测的样本均值](/probability-ml/markov-dependent-sample-mean)：两状态平稳 Markov 链的滞后协方差、精确方差与有效样本量；
- [相关样本的置信区间](/probability-ml/dependent-interval-coverage)：AR(1) 的有限窗口方差、有效样本量与朴素区间欠覆盖实验；
...
- [簇级 Bootstrap：相关用户不能逐条重抽](/probability-ml/cluster-bootstrap-calibration)：按预定义完整用户或设备簇重采样，并公开簇间可交换与配对边界；
- [时间分层簇级 Bootstrap：趋势与用户相关不能各抽各的](/probability-ml/time-stratified-cluster-bootstrap)：冻结时间层内完整簇的重采样，拒绝跨层重复簇并公开纵向依赖边界；
- [失访不是缺失行：观测过程与逆概率加权](/probability-ml/attrition-observation-ipw)：冻结入组队列、声明观测概率，并审计 HT/Hájek 加权的识别边界；
