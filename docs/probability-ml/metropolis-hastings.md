...
目标权重与每个提议概率都必须是有限正实数：`NaN` 不能被视为“既不小于零也不大于零”，无穷也不能进入归一化或行和。这个输入合同不替代对目标密度或提议设计的建模判断，但避免非数值悄悄污染接受率和转移核。

## 平稳转移核不等于有限链已经混合

对同一个两状态核，详细平衡已经成立，但从不同初值走一步仍有不同偏差。下面精确传播分布，而非把一次随机轨迹的频率当作证明：

```python
from projects.naive_bayes_spam.metropolis_hastings import finite_horizon_mixing_report

report = finite_horizon_mixing_report(target, proposal, [0, 1], steps=1, tolerance=0.01)
assert report["paths"][0]["total_variation_distances"][-1] == 0.25
assert abs(report["paths"][1]["total_variation_distances"][-1] - 1 / 12) < 1e-12
assert not report["all_initials_within_tolerance_at_horizon"]
```

这里比较的是到目标分布的总变差距离 $\frac12\sum_x|\mu(x)-\pi(x)|$：它同时显示两条路径仍未达到声明容差，也显示初值影响大小不同。有限状态枚举能诊断这个玩具核，不能给连续高维模型提供混合时间保证；真实分析仍需多链、收敛诊断与问题结构。
...
相关样本意味着 $N$ 个状态不等于 $N$ 个独立样本。应查看轨迹、多条不同初值链、接受率、自相关和有效样本量；单一链的均值稳定或接受率“好看”都不足以证明混合。上面的精确小状态对照也只证明给定步数仍有偏差，不能从一次“已在容差内”外推任意初值或一般状态空间。
...
3. **编码**：运行 `finite_horizon_mixing_report`，比较两个初值的一步总变差距离；篡改其“均已达容差”结论，确认重放证书拒绝。
...
3. 从 0 出发的一步分布是 $(0,1)$，距 $(1/4,3/4)$ 为 $1/4$；从 1 出发为 $(1/3,2/3)$，距目标为 $1/12$。详细平衡不让这两个有限步距离自动为零。
