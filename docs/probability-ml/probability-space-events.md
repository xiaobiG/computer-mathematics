...
证书中可选的比较容差只用于有限浮点舍入差，必须是有限非负数。把容差设为 `inf` 会让任意两个有限字段看起来都“足够接近”，相当于撤销了篡改检查；它既不表达概率的不确定性，也不能替代置信区间、后验或统计模型。对于本页的公平硬币例，所有值可精确写出；对来自浮点加和的模型，容差也应随计算误差的来源明确说明，而不是随意放大。
