...
这个最小实验室不是绘制动画，而是输出算法每次推进后的状态和候选解验证结果。读者可以把每一行轨迹与 BFS 的层级不变量、Kahn 算法的入度不变量或最大流的残量网络对应起来，也可对比 3-SAT 的线性验证与指数搜索。BFS 现在同时保存首次发现父指针，并用父路径、逐边层级不等式与队列重放验证无权最短路；Dijkstra 还提供独立的最短路证书检查：父指针证明可达路径，逐边松弛不等式证明不存在更短路径；Kahn 轨迹则可重放验证每次入度更新、合法拓扑序或剩余正入度的环证据。
