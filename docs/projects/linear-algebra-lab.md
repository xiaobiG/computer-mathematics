...
本项目把线性代数专题的三个关键操作做成小型、可测试的 Python 模块：矩阵乘法、带部分选主元的方程求解，以及向量投影。它们用于核对数学定义，不取代 NumPy/SciPy 的生产实现。
...
测试覆盖矩阵形状错误、非交换变换、选主元、奇异系统与正交投影。完整项目测试仍可通过 `npm run projects:test` 运行。
...
模块仅使用密集 Python 列表和绝对容差，适合小矩阵教学。大型、稀疏或病态问题应采用成熟数值库及相对容差策略。
