...
本项目把线性代数专题的关键操作做成小型、可测试的 Python 模块：矩阵乘法、带部分选主元的方程求解、正规方程与 QR 的最小二乘对照、向量投影、幂迭代、低秩压缩，以及把灰度矩阵展平后的余弦相似度检索。`least_squares_comparison_report` 用同一份 $A^Tr$ 证书检查两条拟合路径；`compressed_image_search` 将压缩误差和压缩域排序放入同一报告，形成微型“压缩—检索”流程。它们用于核对数学定义，不取代 NumPy/SciPy 的生产实现。
...
测试覆盖矩阵形状错误、非交换变换、选主元、奇异系统、正规方程与 QR 的最小二乘一致性及 $A^Tr$ 证书、正交投影、幂迭代残差与失败边界、秩一矩阵重建、精确谱尾误差、低秩参数节省、更高保留秩不增加小例重构误差、压缩—检索联合报告，以及同形图像的余弦检索。完整项目测试仍可通过 `npm run projects:test` 运行。
